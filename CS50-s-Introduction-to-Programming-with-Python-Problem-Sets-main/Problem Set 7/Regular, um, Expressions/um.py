...
appears in that text, case-insensitively, as a word unto 
itself, not as a substring of some other word. For instance, 
...
implement, in a file called test_um.py, three or more functions that 
collectively test your implementation of count thoroughly, each of 
...
def count(s):
    ...
    text=re.split(r"(\W+)", s, flags=re.IGNORECASE)
    cnt=0
    for i in range(len(text)):
        if re.match("um", text[i], flags=re.IGNORECASE):
            cnt += 1
